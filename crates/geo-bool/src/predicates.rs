...
mod tests {
    ...
    fn point_inside() -> Geometry {
        Geometry::Point(Point { x: 0.5, y: 0.5 })
    }
    fn point_outside() -> Geometry {
        Geometry::Point(Point { x: 2.0, y: 2.0 })
    }

    #[test]
    fn test_contains_inside() {
        assert!(contains(&square(), &point_inside()));
    }
    #[test]
    fn test_contains_outside() {
        assert!(!contains(&square(), &point_outside()));
    }
    #[test]
    fn test_intersects() {
        assert!(intersects(&square(), &point_inside()));
    }
    #[test]
    fn test_disjoint() {
        assert!(disjoint(&square(), &point_outside()));
    }
    #[test]
    fn test_within() {
        assert!(within(&point_inside(), &square()));
    }
    #[test]
    fn test_equals() {
        assert!(equals(&square(), &square()));
    }
