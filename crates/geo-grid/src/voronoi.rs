...
pub fn voronoi(points: &[Point], bbox: &BBox) -> Vec<Polygon> {
    ...
    for (_, cell_list) in cells {
        ...
        for &(ci, cj) in &cell_list {
            ...
            if is_boundary {
                boundary.push(Point { x: bx + (ci as f64 + 0.5) * cell_w, y: by + (cj as f64 + 0.5) * cell_h });
            }
        }
        ...
        boundary.sort_by(|a, b| (a.y - cy).atan2(a.x - cx).partial_cmp(&(b.y - cy).atan2(b.x - cx)).unwrap());
